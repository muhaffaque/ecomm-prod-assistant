    ...
    def scrape_flipkart_products(self, query, max_products=1, review_count=2):
        ...
        for item in items:
            try:
                ...
                # LINK + TITLE (stable)
                link_el = item.find_element(By.XPATH, ".//a[contains(@href, '/p/')]")
                product_link = link_el.get_attribute("href")

                title = link_el.get_attribute("title") or link_el.text
                ...
                # RATING
                try:
                    rating = item.find_element(By.XPATH, ".//div[contains(text(),'.')]").text
                    if len(rating) > 5:
                        rating = "N/A"
                except:
                    rating = "N/A"
                ...
                try:
                    text = item.text
                    match = re.search(r"(\d{1,3}(,\d{3})*)\s+Ratings", text)
